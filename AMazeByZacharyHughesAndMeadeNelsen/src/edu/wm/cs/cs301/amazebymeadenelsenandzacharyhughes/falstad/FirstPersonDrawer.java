...
public class FirstPersonDrawer extends DefaultViewer {
	...
	Canvas canvas ; // the graphics object for the buffer image this class draws on
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root, Maze maze){
		...
		dummyMazePanel = this.maze.panel; 
		angle = 0 ;
	 ...
	 * @deprecated Use {@link falstad.MazePanel#redraw(edu.wm.cs.cs301.amazebymeadenelsenandzacharyhughes.falstad.FirstPersonDrawer,Graphics,int,int,int,int,int,int,int,RangeSet,int)} instead
	 */
	
	@Override
	public void redraw(Canvas canvas, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rangeset, int ang) {
		if (rangeset == null)
			System.out.println("rset is null in first person drawer");
		this.dummyMazePanel.dummyRedraw(this, canvas, state, px, py, view_dx, view_dy,
			walk_step, view_offset, rangeset, ang);
	 ...
	 * @param nn
	 */
	void drawAllVisibleSectors(BSPNode nn) {
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
//		this.dummyMazePanel.dummySetColor(gc, seg.col);
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			this.dummyMazePanel.dummyFillPolygon(canvas, xps, yps, 4);
//			canvas.fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
