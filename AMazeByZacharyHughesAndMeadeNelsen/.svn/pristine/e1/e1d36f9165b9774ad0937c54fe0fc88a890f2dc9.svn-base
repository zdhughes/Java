 ...
 */
public class MazeBuilder extends AsyncTask<Void, Void, Void>{
	...
	static void dbg(String str) {
		//COME BACK IF WEBCAT RAISES AN ISSUE.
	...
	protected Void doInBackground(Void... params) {
		// try-catch block to recognize if thread is interrupted
				try {
					...
					if(maze.saveMaze.equals("true")){
						System.out.println("Saved a MAZE");
						System.out.println("The difficulty before establishing a new context is: " + String.valueOf(this.maze.difficulty)); 
					    MazeFileWriter writeMaze = new MazeFileWriter();
					    Context writingContext = maze; 
					    System.out.println("Writing too:" + writingContext.getFilesDir() + "maze" + maze.difficulty +".xml"); 
					    
					    writeMaze.store(writingContext.getFilesDir() + "maze" + maze.difficulty +".xml", width, height, rooms, expectedPartiters, root, cells, dists.getDists()
							    , startx, starty);
					}
